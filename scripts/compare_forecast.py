"""Does the frozen forecaster buy back the gridlock in the fast-shifting regime (SCN-08)?

On SCN-08 the plain DQN beats Webster on wait (~1.8s vs 2.6s) but pays 20-40% gridlock (vs
Webster 0%) - its aggressiveness tips recoverable jams at FEASIBLE load. The forecaster's job:
anticipate the 300s anti-phase shift (60/90/120s horizon) and pre-empt the jam -> keep the wait
edge AND cut gridlock toward Webster's 0%. This is the regime the forecast was made for (the prior
'no benefit' calls were on steady/near-steady scenarios). No retrain: reuses the trained hybrid
ep299 checkpoints + the official frozen LSTM.

Per-seed x per-scenario split. References: webster + bare plain.

Run:: LIBSUMO_AS_TRACI=1 python -m scripts.compare_forecast --scenarios SCN-08
"""
...
_OUT = _REPO_ROOT / "data" / "eval" / "compare_forecast"
SEEDS = (42, 123, 2024)
...
def main() -> None:
    ...
    p.add_argument("--scenarios", nargs="+", default=["SCN-08"])
    ...
    for scn_id in args.scenarios:
        ...
        for name in order:
            ...
            print(f"  {name:13} wait={w:6.2f} | thru={t:7.1f} | grid={g:4.0f}%", flush=True)
