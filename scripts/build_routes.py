...
def _route_xml(scenario: Scenario, seed: int, trips: list[Trip]) -> str:
    ...
    lines = [
        ...
        "<routes>",
        # Junction-model params for the yielding ('g') free rights - both needed to
        # stop the ~1-per-100-episodes merge collision (decisions.md 2026-08-28):
        # - jmTimegapMinor=3.0: demand a human-plausible gap (HCM ~4.5-6s; SUMO
        #   default 1.0s) before passing ahead of a prioritized foe.
        # - jmAdvance=0 (jm params parse as floats; 0=false): while unable to
        #   complete the pass, wait BEFORE the junction instead of creeping onto
        #   the curve. The traced collision was a right-turner whose clearance
        #   estimate went stale mid-entry (the foe, already on the junction,
        #   accelerated); it emergency-stopped with its nose in the conflict zone
        #   and was clipped.
        f'    <vType id="{scenario.vehicle_type}" vClass="passenger" '
        'jmTimegapMinor="3.0" jmAdvance="0"/>',
    ]
