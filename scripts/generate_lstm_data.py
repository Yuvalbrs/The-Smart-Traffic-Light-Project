...
def _sha256_file(path: Path) -> str:
    """Content digest of one corpus CSV (the manifest's byte-level identity)."""
    h = hashlib.sha256()
...
def main() -> None:
    ...
    parser.add_argument(
        "--manifest-only",
        action="store_true",
        help="rebuild manifest.json from the CSVs already on disk, without re-simulating. Safe "
             "because generation is deterministic per (scenario, seed) and data_version is not "
             "an input to the CSV, so the bytes cannot differ.",
    )
    ...
    for scenario in scenarios:
        ...
        for seed in seeds:
            if args.manifest_only:
                scn_num = scenario.id.split("-")[1]
                path = _OUT_DIR / f"scn_{scn_num}_seed_{seed:02d}.csv"
                if not path.exists():
                    print(f"!! --manifest-only: {path.name} missing; run generation first")
                    sys.exit(1)
            ...
            dv = data_version(
                ...
                sumo_version=sumo_v,
                # WITHOUT this every scenario at a given seed minted the SAME data_version -
                # 100 files, 10 ids. The chain in preregistration s9 needs an id that
                # distinguishes SCN-01 from SCN-10.
                dataset_key=scenario.id,
            )
            manifest.append(
                {
                    "file": path.name,
                    ...
                    "n_rows": n_rows,
                    "data_version": dv,
                    # The id above is derived from CONFIG; this is derived from the BYTES.
                    # Only the latter can catch a file that was regenerated by a different
                    # world but happens to carry matching config inputs.
    ...
    verb = "indexed" if args.manifest_only else "wrote"
    n_dv = len({e["data_version"] for e in manifest})
    n_sha = len({e["file_sha256"] for e in manifest})
    print(f"[lstm-data] OK - {verb} {total} CSV(s) + manifest -> {manifest_path.relative_to(_REPO_ROOT)}")
    print(f"[lstm-data] {len(manifest)} files / {n_dv} distinct data_versions / {n_sha} distinct sha256")
    if n_dv != len(manifest):
        print("!! data_version collision - an id does not identify a file", file=sys.stderr)
        sys.exit(2)
    sys.exit(0)
